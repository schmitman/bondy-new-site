    ...
    <main style={{ backgroundColor: tw.bg, backgroundImage: notebookBg, minHeight: '100vh' }}>
      ...
      <header style={{ background: tw.white, borderBottom: `1px solid ${tw.rule}` }}>
        <HeroV2
          ...
          title={
            <>
              {a.h1_1}<br />{a.h1_2} <em style={{ fontStyle: 'normal', color: tw.green }}>{a.h1_em}</em>
            </>
          }
          underlineWidth={260}
          body={a.intro}
          meta={[
            { n: '2008', label: lang === 'es' ? 'Fundada' : 'Founded' },
            { n: '18', label: lang === 'es' ? 'Años' : 'Years' },
            { label: 'Buenos Aires', separatorBefore: true },
            { label: lang === 'es' ? 'Marca v4 · 2026' : 'V4 brand · 2026' },
          ]}
        />
      </header>
